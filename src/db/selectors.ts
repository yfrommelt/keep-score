...
import { db, GameEntity, PlayerEntity, RoundScoreEntity } from "./db";
...
async function enhanceGameWithPlayers(game: GameEntity): Promise<Game> {
  const gameScores = await db.gameScores
    .where({ gameId: game.id })
    .sortBy("order");
  const players = await Promise.all(
    gameScores.map(async (gameScore) => {
...
export function usePlayers(): PlayerEntity[] {
  return useLiveQuery(async () => {
