...
export async function createGame(playerNames: string[], name?: string) {
    const gameId = await db.games.add({name, createdAt: new Date(), updatedAt: new Date()});
...
async function updateGameScore(gameId: number, playerId: number) {
    const roundScores = await db.roundScores.where({gameId, playerId}).toArray();
    const gameScore = roundScores.reduce((acc, roundScore) => acc + roundScore.value, 0);
...
export async function addPlayerScore(gameId: number, playerId: number, value: number) {
    await db.transaction('rw', db.roundScores, db.gameScores, db.games, async () => {
        await db.roundScores.add({gameId, playerId, value, updatedAt: new Date()});
...
export async function deletePlayerLastScore(gameId: number, playerId: number) {
    await db.transaction('rw', db.roundScores, db.gameScores, db.games, async () => {
        const lastRoundScore = await db.roundScores.where({gameId, playerId}).last();
        if (!lastRoundScore) {
            return;
        }
